$(document).ready(function(){
	...
	var mmedia = {
		...
		getData : function (tipe, from, start) {
			...
			var urlget = BASEURLS+'/'+tipe_call+'/listAjax/';
...
//					$('.previewTarget').closest('.thumbnail').append(img_add);
		...
		insertFile : function (from, data, sr_img) {
			...
			a_view		= $('<a/>').html('View').attr({href: BASEURLS+"/gadmin/media_manager/viewapp/"+data.id, target: '__blank'});
	...
	var imx = {
		...
		uploadFiles : function (event) {
			...
			data.append(token_name, csrf_hash);
			 ...
			 $.ajax({
			     url: BASEURLS+'/gadmin/media_manager/uploadAjax?files',
			     type: 'POST',
			     data: data,
			     cache: false,
			     dataType: 'json',
		...
		submitForm : function (event, data) {
			...
			$.ajax({
				url: BASEURLS+'/gadmin/media_manager/uploadAjax',
		        type: 'POST',
		        data: formData,
		        cache: false,
		        dataType: 'json',
